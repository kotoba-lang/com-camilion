(ns camilion.main
  "Kotodama WASM entrypoint for the Camilion clean-room actor (L4) — Clojure port.
  ...
  facts (`camilion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "camilion")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "camilion_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}        :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "camilion_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"camilion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "camilion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :camilion-compat/ready)
